...
void Enemy::Initialize()
{
  ...
  m_Blackboard =
    ai::CreateBlackboard(globals::GetAssetDirectory() + "blackboards/ad.csv");
...
void Enemy::Update(const float delta_time)
{
  ...
  m_Transform.Position.x = std::clamp(
    m_Transform.Position.x, 0.f, screen_dimensions.w - 32.f); // Offsetting image size
  m_Transform.Position.y = std::clamp(
    m_Transform.Position.y, -16.f, screen_dimensions.h - 64.f); // Offsetting image size
...
{
  // TODO @RyanWestwood : Neaten entire project to use Vector2 over SDL_Point where
  // needed.
  //						clean this code!
  Vector2 start_pos = {(int)m_Transform.Position.x / 32 + 1,
                       (int)m_Transform.Position.y / 32 + 1};
  Vector2 new_goal  = {(int)goal.x, (int)goal.y};
  m_Path            = pathing::CreatePath(
    start_pos, new_goal, pathing::Algo::A_Star, ai::path::Obstacle::Ad);
  m_SmoothedPath->UpdatePath(
    m_Path, start_pos, 5.f, m_StoppingDistance); // TODO: fix this.
}
...
void Enemy::GoalTile()
{
  if(m_Path.size() <= 1)
    return;
  auto current_position = {(int)m_Transform.Position.x / 32 + 1,
                           (int)m_Transform.Position.y / 32 + 1};
...
{
  if(m_GoalTile.x == 0.f && m_GoalTile.y == 0.f)
    return;

  SDL_FPoint GreenBox   = {(int)m_Transform.Position.x / 32 + 1,
                         (int)m_Transform.Position.y / 32 + 1};
...
void Enemy::FollowSmoothedPath(const float delta_time)
{
  ...
  Vector2 position_2d = {(int)m_Transform.Position.x / 32,
                         (int)m_Transform.Position.y / 32};
  ...
  if(following_path)
  {
    if(path_index >= m_SmoothedPath->m_SlowDownIndex && m_StoppingDistance > 0)
    {
      ...
      float           distance_from_point =
        line.DistanceFromPoint(position_2d) / m_StoppingDistance;
    ...
    const float angle_degrees =
      std::atan2f(dir.y, dir.x) * (180.0 / 3.141592653589793238463);
    m_Transform.Rotation = std::lerp(
      m_Transform.Rotation, angle_degrees, m_Transform.RotationSpeed * delta_time);
    m_Transform.Position.x +=
      std::cos(angle_degrees) * m_Transform.Velocity.x * speed_percentage * delta_time;
    m_Transform.Position.y +=
      std::sin(angle_degrees) * m_Transform.Velocity.y * speed_percentage * delta_time;
  }
}
...
void Enemy::CreateAttackFsm()
{
  ...
  m_AttackEntryState =
    ai::fsm::CreateState(m_AttackFsm, "Attack Entry Point", [&](const float delta_time) {
      if(*m_Timer >= 1.f)
      {
        *m_Timer = 0;
  ...
  m_RangedAttackState =
    ai::fsm::CreateState(m_AttackFsm, "Ranged Attack", [&](const float delta_time) {
      if(*m_Timer >= 1.f)
      {
        *m_Timer = 0;
#ifdef LOGGING
  ...
  m_MeleeAttackState =
    ai::fsm::CreateState(m_AttackFsm, "Melee Attack", [&](const float delta_time) {
      if(*m_Timer >= 3.f)
      {
        *m_Timer = 0;
#ifdef LOGGING
