...
void Cooldown::Initialize(SDL_Point position, int timer, const char* ability_icon)
{
  ...
  m_Foreground.m_Destination = {
    position.x + FPO, position.y + FPO, TILE_SIZE - FSO, TILE_SIZE - FSO};
...
void Cooldown::Update(const float delta_time)
{
  if(m_IsCountingDown)
  {
    ...
    m_Text.m_Dimensions.x =
      m_Background.m_Destination.x - TXO + (TILE_SIZE - m_Text.m_Dimensions.w / 2) / 2;
    m_Text.m_Dimensions.y =
      m_Background.m_Destination.y - TYO + (TILE_SIZE - m_Text.m_Dimensions.h / 2) / 2;
