...
namespace font
{
  ...
  FontData LoadFont(const char*      filename,
                    int              fontsize,
                    const char*      message,
                    ...
                    const SDL_Point& position)
  {
    ...
    {
      TTF_Font* font =
        TTF_OpenFont((g_FontDirectory + std::string(filename)).c_str(), fontsize);
      if(font == nullptr)
        throw FontError();
  ...
  FontData UpdateMessage(const char*      message,
                         TTF_Font*        font,
                         const SDL_Color& color,
                         const SDL_Point& position)
  {
...
{
  auto font_data =
    font::UpdateMessage(message, font::GetDefaultFont().m_Font, m_Colour, m_Position);
  m_Message    = message;
  m_Font       = font_data.m_Font;
  m_Texture    = font_data.m_Texture;
  m_Dimensions = font_data.m_Dimensions;
