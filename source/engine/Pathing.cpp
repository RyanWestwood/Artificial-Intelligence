...
namespace pathing
{
  ...
  bool InitializePathing()
  {
    ...
    g_DebugTextureExploredData =
      texture::LoadSolidColourTexture({255, 255, 255, 255}, {32, 32});
    ...
    for(int y = 0; y < tilemap_dimensions.h; y++)
    {
      for(int x = 0; x < tilemap_dimensions.w; x++)
      {
        ...
        node.m_Destination = {
          tile_size.w * x + 8, tile_size.h * y + 8, tile_size.w / 2, tile_size.h / 2};
  ...
  std::vector<Vector2>
  CreatePath(Vector2 start, Vector2 end, Algo algorithm, ai::path::Obstacle layer)
  {
    auto tilemap_dimensions = globals::GetTileMapDimensions();

    ai::path::NodePtr start_node =
      g_NodePtrs.at(start.x + (start.y * tilemap_dimensions.w));
    ...
    switch(algorithm)
    {
      ...
      case pathing::Algo::IDDFS:
        timer::StartTimer();
        solution_path =
          ai::path::IDDFS_Caller(g_NodePtrs, start_node, end_node, layer, 25);
        timer::StopTimer("IDDFS");
        break;
      default: break;
    }
#ifdef LOGGING
    g_SolutionNodes.insert(
      std::end(g_SolutionNodes), std::begin(solution_path), std::end(solution_path));
  ...
  void DebugPaths(globals::Vector& tilemap_dimensions, globals::Vector tile_size)
  {
    for(int y = 0; y < tilemap_dimensions.h; y++)
    {
      for(int x = 0; x < tilemap_dimensions.w; x++)
      {
        g_Nodes[x + (y * tilemap_dimensions.w)].m_TextureData =
          g_NodePtrs.at(x + (y * tilemap_dimensions.w))
            ->IsObstacle(ai::path::Obstacle::All)
          ? g_DebugObstacle
          : g_DefaultTexture;
      }
    }
    ...
    {
      g_Nodes.at(node.x + (node.y * tilemap_dimensions.w)).m_TextureData.m_Texture =
        g_DebugTextureStartData.m_Texture;
    }
    for(auto& node : g_EndNodes)
    {
      g_Nodes.at(node.x + (node.y * tilemap_dimensions.w)).m_TextureData.m_Texture =
        g_DebugTextureEndData.m_Texture;
    }
  }
