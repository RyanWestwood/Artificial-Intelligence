...
void Sword::Update(const float& delta_time, const SDL_FPoint position)
{
  ...
  if(m_Action)
  {
    m_Rotation = std::clamp(
      m_Rotation + side * delta_time * SWING_SPEED, -SWING_ROTATION, SWING_ROTATION);
  ...
  {
    m_FireOffset.x =
      std::clamp(m_FireOffset.x + side_flipped * delta_time * 30, -1000.f, 1000.f);
...
void Sword::Fire()
{
  if(*m_Timer > m_Cooldown)
  {
    m_Projectile.Activate(
      {float(m_Sprite.m_Destination.x), float(m_Sprite.m_Destination.y)}, m_Facing);
    m_FireAction = true;
    *m_Timer     = 0.f;
  }
}
