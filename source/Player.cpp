...
{
  if(m_Transform.Velocity.y < 0)
    *m_Facing = globals::Direction::North;
  if(m_Transform.Velocity.x > 0)
    *m_Facing = globals::Direction::East;
  if(m_Transform.Velocity.y < 0 && m_Transform.Velocity.x > 0)
    *m_Facing = globals::Direction::NorthEast;
  if(m_Transform.Velocity.y > 0)
    *m_Facing = globals::Direction::South;
  if(m_Transform.Velocity.y > 0 && m_Transform.Velocity.x > 0)
    *m_Facing = globals::Direction::SouthEast;
  if(m_Transform.Velocity.x < 0)
    *m_Facing = globals::Direction::West;
  if(m_Transform.Velocity.y > 0 && m_Transform.Velocity.x < 0)
    *m_Facing = globals::Direction::SouthWest;
  if(m_Transform.Velocity.y < 0 && m_Transform.Velocity.x < 0)
    *m_Facing = globals::Direction::NorthWest;
  if(m_Transform.Velocity.x == 0 && m_Transform.Velocity.y == 0)
    *m_Facing = globals::Direction::None;

  auto screen_dimensions = globals::GetScreenDimensions();
  m_Transform.Position.x = std::clamp(
    m_Transform.Position.x + static_cast<float>(m_Transform.Velocity.x) * delta_time,
    0.f,
    screen_dimensions.w - 32.f); // Offsetting image size
  m_Transform.Position.y = std::clamp(
    m_Transform.Position.y + static_cast<float>(m_Transform.Velocity.y) * delta_time,
    -16.f,
    screen_dimensions.h - 64.f); // Offsetting image size
  ...
  SDL_FPoint sword_position = {m_Transform.Position.x + ((!m_Image.FlipSprite << 5) - 14),
                               m_Transform.Position.y +
                                 2}; // Sets the sword left or right of the player
